...
public class DateUtil {
	...
	public String getPreviousYearEnd() {
		...
		int yearPlus = this.getYearPlus();
		GregorianCalendar currentDate = new GregorianCalendar();
		currentDate.add(Calendar.DATE, yearPlus + MaxYear * weeks
				+ (MaxYear - 1));
		Date yearDay = currentDate.getTime();
		DateFormat df = DateFormat.getDateInstance();
		String preYearDay = df.format(yearDay);
...
//	 获得参数月的本季度第一天
	...
	public static void main(String[] args) {
		String hm = "07:00";
		int start = Integer.valueOf("07:00".substring(0, 2));
		int end = Integer.valueOf("20:00".substring(0, 2));
		JSONArray ja = new JSONArray();
		for (int i = 0; i < (end-start)*2; i++) {
			JSONObject jb = new JSONObject();
			String format = "yyyy-MM-dd HH:mm";
			Calendar cal = Calendar.getInstance();
			cal.setTime(DateUtil.StringtoDate("2022-02-01 "+hm, format));
			cal.add(Calendar.MINUTE, 30*i);
			Date d2 = cal.getTime();
			cal.add(Calendar.MINUTE, 30);
			Date d3 = cal.getTime();
			//System.out.println(DateUtil.DateToString(d2, "yyyy-MM-dd HH:mm"));
			jb.put("id", i);
			if(i%2==0) {
				jb.put("time", DateUtil.DateToString(d2, "HH:mm"));
			}else {
				jb.put("time", "");
			}
			jb.put("text", DateUtil.DateToString(d3, "HH:mm"));
			jb.put("clazz", "blank");
			ja.add(jb);
		}
    ...
    * @param start2 开始时间2
    * @param end2   结束时间3
    * @return       true无交集,false有交集
    */
   public static boolean compare2Times(String start1,String end1,String start2,String end2){
   	boolean flag = false;
   	if(DateUtil.TimeSub(start1, end2) > 0 
   	   && DateUtil.TimeSub(end1, start2) > 0){
   		flag = true;
   	}
   	return flag;
   }
   ...
   /***
    * 比较一个时间段,是否合理,即结束时间>开始时间
    * @param start 开始时间
    * @param end   结束时间
    * @return      true大于,false不大于
    */
   public static boolean compare2Time(String start,String end){
   	boolean flag = false;
   	if(DateUtil.TimeSub(start, end) > 0){
   		flag = true;
   	}
   	return flag;
   }
